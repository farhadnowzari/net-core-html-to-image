...
namespace CoreHtmlToImage
{
	...
	public class HtmlConverter
	{
		...
		{
			return FromHtmlString( html, options => { } );
		}
		...
		/// <returns></returns>
		public byte[] FromHtmlString( string html, Action<ConverterOptions> options )
		{
			var filename = Path.Combine( directory, $"{Guid.NewGuid()}.html" );
			File.WriteAllText( filename, html );
			var bytes = FromUrl( filename, options );
		...
		/// <param name="quality">Output image quality 1-100</param>
		/// <returns></returns>
		public byte[] FromUrl( string url, Action<ConverterOptions> options )
		{
			var converterOptions = new ConverterOptions();
			options.Invoke( converterOptions );
			var filename = Path.Combine( directory, $"{Guid.NewGuid()}.{converterOptions.OutputFormatString}" );

			string args = converterOptions.GetCommandArgs();

			if ( IsLocalPath( url ) )
			{
				args = $"{args} \"{url}\" \"{filename}\"";
			}
			else
			{
				args = $"{args} {url} \"{filename}\"";
